...
| Finding | Consequence |
| --- | --- |
| `CreateCornerRectangle` dimensions its own rectangle. Immediately after the call, before `AddDimension2` is used at all, the sketch already holds two dimensions (`D1`, `D2`). | Dimensions added afterwards are a *second* pair on the same two edges. Ours report driving and the tool's already fix the geometry, so driving ours moves nothing. Delete what the tool created before adding your own; `swhelpers.clear_sketch_dimensions` does this. |
